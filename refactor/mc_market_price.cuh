...
__global__ void market_price(float* P0_sum, curandState* states, float r0){
...
inline void simulate_market_price(float* h_P, curandState* d_states, float r0){
    float* d_P0_sum;
    cudaMalloc(&d_P0_sum, N_MAT * sizeof(float));
    cudaMemset(d_P0_sum, 0, N_MAT * sizeof(float));

    market_price<<<NB, NTPB>>>(d_P0_sum, d_states,r0);
